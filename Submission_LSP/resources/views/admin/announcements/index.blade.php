        ...
        @forelse($announcements as $announcement)
            <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden flex flex-col">
                @if ($announcement->image)
                    <img src="{{ Storage::url($announcement->image) }}" alt="{{ $announcement->judul }}" class="h-48 w-full object-cover">
                @else
                ...
                <div class="p-5 flex-1 flex flex-col">
                    <h3 class="text-lg font-bold text-slate-800 mb-2">{{ $announcement->judul }}</h3>
                    <p class="text-sm text-slate-600 line-clamp-3 mb-4 flex-1">{{ $announcement->isi }}</p>
                        ...
                        <div class="flex space-x-2">
                            ...
                            <form action="{{ route('admin.announcements.destroy', $announcement->id) }}" method="POST"
                                ...
                                <button type="submit" class="p-2 text-red-600 hover:bg-red-50 rounded transition-colors"
                                    title="Hapus" onclick="return confirm('Hapus pengumuman ini?')">
