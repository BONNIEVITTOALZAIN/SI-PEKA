                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    @forelse($pemeriksaans as $pemeriksaan)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="p-4 text-slate-500 max-w-xs truncate" title="{{ $pemeriksaan->keluhan }}">
                                {{ $pemeriksaan->keluhan }}</td>
                            <td class="p-4">
                            ...
                            <td class="p-4 text-center">
                                @if ($pemeriksaan->catatan)
                                    <span class="px-2.5 py-1 text-xs font-medium text-slate-700 rounded-full"
                                        title="{{ $pemeriksaan->catatan }}">{{ $pemeriksaan->catatan }}</span>
                                @else
                            ...
                            <td class="p-4 flex justify-center space-x-2">
                                @if ($pemeriksaan->status == 'pending')
                                    <form action="{{ route('admin.pemeriksaan.verify', $pemeriksaan->id) }}" method="POST"
                                        class="flex flex-col space-y-2 w-full min-w-[150px]">
                                        @csrf
                                        <input type="text" name="catatan" placeholder="Catatan"
                                            class="text-xs rounded-md border border-slate-300 px-2 py-1 w-full focus:ring-blue-500 focus:border-blue-500">
                                        <div class="flex justify-center space-x-2">
                                            <button type="submit" name="status" value="diterima"
                                                class="flex-1 p-1.5 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded transition-colors"
                                                title="Terima">
                                                <i class="fas fa-check text-xs"></i> Terima
                                            </button>
                                            <button type="submit" name="status" value="ditolak"
                                                class="flex-1 p-1.5 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white rounded transition-colors"
                                                title="Tolak">
