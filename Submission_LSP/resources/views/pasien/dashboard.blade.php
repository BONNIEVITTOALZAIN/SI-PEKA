            ...
            <div class="space-y-4">
                ...
                <div class="flex space-x-4">
                    ...
                    <div class="flex-shrink-0">
                        <img src="{{ Storage::url($announcement->image_path) }}" alt="Gambar" class="h-20 w-20 object-cover rounded-lg border border-slate-200">
                    </div>
                    @endif
                    <div>
                        <h3 class="text-md font-semibold text-blue-600">{{ $announcement->title }}</h3>
                        <span class="text-xs text-slate-400 block mb-1">{{ $announcement->created_at->format('d M Y') }}</span>
    ...
    <div class="space-y-6">
        <div class="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl shadow-md p-6 text-white relative overflow-hidden">
            <div class="relative z-10">
                ...
                <a href="{{ route('pasien.pemeriksaan.index') }}" class="inline-block bg-white text-blue-700 font-medium px-4 py-2 rounded-lg shadow hover:bg-blue-50 transition-colors">
                    Daftar Sekarang
                </a>
            </div>
            ...
            <ul class="space-y-3">
                ...
                <li class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 truncate w-32" title="{{ $p->keluhan }}">{{ $p->keluhan }}</p>
                    </div>
                    <div>
                        @if($p->status == 'pending')
                            <span class="px-2 py-1 text-[10px] font-medium bg-orange-100 text-orange-700 rounded-full">Pending</span>
                        @elseif($p->status == 'diterima')
                            <span class="px-2 py-1 text-[10px] font-medium bg-blue-100 text-blue-700 rounded-full">Diterima</span>
                        @elseif($p->status == 'selesai')
                            <span class="px-2 py-1 text-[10px] font-medium bg-green-100 text-green-700 rounded-full">Selesai</span>
                        @else
                            <span class="px-2 py-1 text-[10px] font-medium bg-red-100 text-red-700 rounded-full">Ditolak</span>
                        @endif
                    </div>
                </li>
                @empty
